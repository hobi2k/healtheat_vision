...
from .my_sam import refine_boxes_with_sam
from src.config import Config, init_logger

logger = init_logger()

# 경로 및 리소스 로드 (파이프라인의 입력 부분)
IMG_DIR = Config.DATA_DIR / "clean_data" / "traiimages"
ANN_DIR = Config.DATA_DIR / "clea_data" / "train_annotations"
OUT_DIR = Config.DATA_DIR / "unified_dataset_v2" / "annotations"
OUT_DIR.mkdir(parents=True, exist_ok=True)

# YOLO 모델 로드
YOLO_MODEL = YOLO(str(Config.BASE_DIR / "outputs/models/hobi_yolov8_pill_2u2/weights/best.pt"))
...
"""
unpacking: ax1, ay1, ax2, ay2 = a  
IoU = intersection area / union area  
  
[입력] 두 개의 bbox (xyxy 형태)  
[출력] float (0~1)  
  
[구조] GT vs YOLO 비교 auto_correct()의 핵심 지표.  
IoU가 높을수록 같은 객체일 가능성이 높다.  
#  
[주의] bbox가 완전히 겹치지 않으면 inter_w 또는 inter_h가 음수가 될 수 있어 max(0)을 취한다.  
union이 0이면 divide by zero 방지 위해 0 반환.
"""
...
# YOLO 모델로 박스 검출 -> COCO category_id/name 붙여 반환
...
def detect_boxes(img_path: Path, conf_th: float = 0.1):
    img = cv2.imread(str(img_path)) # cv2.imread(): 문자열 경로 필요 
    if img is None:
        logger.warning(f"[WARN] 이미지 로드 실패: {img_path}")
        return []

    # [설명] YOLO_MODEL(img) -> inference  
    # 결과는 리스트이며, 이미지 하나만 넣었으므로 [0] 선택  
    #  
    # [입출력]  
    # 입력: numpy array  
    # 출력: Results 객체  
    # Results.boxes.xyxy, .cls, .conf 등을 포함
    ...
    # [문법] zip을 사용하여 bbox와 cls를 동일 인덱스로 묶음  
    #  
    # [입출력]  
    # 입력: YOLO bbox, YOLO class  
    # 출력: dets 리스트(dict)      
...
def main():
    ...
    for img_path in imgs:
        ...
        # SAM으로 bbox 정밀화
        det_boxes = refine_boxes_with_sam(img_path, det_boxes)       
